    ...
    const fetchHistoricalPlaces = async () => {
      try {
        const response = await axios.post('http://localhost:5000/groq-api', {
          messages: [
            { role: 'system', content: 'Recommend the tourist a list of metropolitan and places with good citylife in India. ' }
          ],
    ...
    try {
      const response = await axios.post('http://localhost:5000/groq-api', {
        messages: [
          {
            role: 'user',
            content: `Plan a day-wise trip from ${placefrom} to the tourist attractions in ${placeto}(specifically focus on the night/city lifestyle of that place as well) with a budget of ${budget}, for ${days} days, for ${people} people. Make the trip affordable according to budget as much as possible. Divide the entire below trip in ${days} days iterenary:
