    ...
    const fetchHistoricalPlaces = async () => {
      try {
        const response = await axios.post('http://localhost:5000/groq-api', {
          messages: [
            { role: 'system', content: 'Recommend the tourist a list of the different religious destinations in India. ' }
          ],
    ...
    try {
      const response = await axios.post('http://localhost:5000/groq-api', {
        messages: [
          {
            role: 'user',
            content: `Plan a day-wise trip from ${placefrom} to the religious destinations in ${placeto} (also recommend user some other tourist destinations in the area) with a budget of ${budget}, for ${days} days, for ${people} people. Make the trip affordable according to budget as much as possible. Divide the entire below trip in ${days} days iterenary:
