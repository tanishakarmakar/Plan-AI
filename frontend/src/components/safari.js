    ...
    const fetchHistoricalPlaces = async () => {
      try {
        const response = await axios.post('http://localhost:5000/groq-api', {
          messages: [
            { role: 'system', content: 'Recommend the tourist a list of wildlife and safari destinations in India. ' }
          ],
    ...
    try {
      const response = await axios.post('http://localhost:5000/groq-api', {
        messages: [
          {
            role: 'user',
            content: `Plan a day-wise trip from ${placefrom} to the wildlife and safari destinations in ${placeto} with a budget of ${budget}, for ${days} days, for ${people} people. Make the trip affordable according to budget as much as possible. Divide the entire below trip in ${days} days iterenary:
